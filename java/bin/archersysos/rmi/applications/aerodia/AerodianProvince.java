...
public class AerodiaProvince extends UnicastRemoteObject 
  ...
  transient Vector names = new Vector();  // names of aerodians in this place
  transient Vector aerodians = new Vector(); // the RemoteAerodian objects
  ...
  /** This remote method returns a Vector of names of aerodians in this place */
  ...
  /** 
   * This remote method returns a RemoteAerodian object corresponding to
  ...
  /** 
   * This remote method moves the specified RemoteAerodian from this place
  ...
  /** 
   * This remote method sends a message to everyone in the room.  Used to
   * do things that aerodians can see.  Requires that the actor be in this place.
   **/
  public void act(RemoteAerodian actor, String msg) 
       throws RemoteException, NotThere {
   ...
   **/
  public void createThing(RemoteAerodian creator,
   ...
   * such thing here.
   **/
  public void destroyThing(RemoteAerodian destroyer, String thing) 
  ...
  /**
   * Create a new place in this Aerodia, with the specified name an description. 
  ...
  /**
   * Create a new exit from this Aerodia, linked to a named place in a named
   * Aerodia on a named host (this can also be used to link to a named place in 
   * the current Aerodia, of course).  Because of the possibilities of deadlock,
   ...
   **/
  public void linkTo(RemoteAerodian linker, String exit, 
                     String hostname, String Aerodianame, String placename) 
    ...
    // Check that the link target actually exists.  Throw NoSuchProvince if not.
    // Note that NoSuchProvince may also mean "NoSuchAerodia" or "AerodiaNotResponding".
    ...
    tellEveryone(name + " has linked " + exit + " to " + 
                 "'" + placename + "' in Aerodia '" + Aerodianame + 
                 "' on host " + hostname);
  }
   ...
   * call it to initially place a person into the Aerodia.  Once the person
   * is in the Aerodia, however, the client should restrict them to using go()
  ...
  public void enter(RemoteAerodian who, String name, String message) 
    ...
    synchronized (names) {
      if (aerodians.indexOf(who) != -1) throw new AlreadyThere();
  ...
  /**
   * This final remote method returns the server object for the Aerodia in which
   * this place exists.  The client should not allow the user to invoke this
   * method.
   **/
  public RemoteAerodiaServer getServer() throws RemoteException { return server; }
  ...
  protected void tellEveryone(final String message) {
    ...
    // Make a copy of the aerodians here now.  The message is sent asynchronously
    // and the list of aerodians in the room may change before the message is
    // sent to everyone.
    ...
    new Thread() {
      public void run() {
        ...
        for(int i = 0; i < recipients.size(); i++) {
          RemoteAerodian person = (RemoteAerodian)recipients.elementAt(i);
   ...
   * aerodians and of their names are transient, they are not serialized with
  ...
  private void readObject(ObjectInputStream in) 
    ...
    aerodians = new Vector();   // and recreate the aerodians vector
  }                     
