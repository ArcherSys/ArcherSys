...
def clean_qs(d):
    return dict((k, v[0] if len(v) == 1 else v) for (k, v) in d.items())
...
def check_mock(response, mock_urlopen, method, url, headers, post_params=None, body=None, raw_response=False):
    ...
    data off the socket.
    """
    body = urllib.parse.urlencode(post_params) if post_params else body
...
class TestGet(unittest.TestCase):
    ...
    def test_crazy_unicode(self):
        json_data = {'foo' : 'bar', 'baz' : 42}
        url = 'https://api.dropbox.com/metadata\u4545\u6f22/\u8a9e'
...
class TestPost(unittest.TestCase):
    ...
    def test_post_params_crazy_unicode_values(self):
        ...
        self._post_params({'quux' : 'is\\u4545', 'a' : 'horse\\u4545'})
