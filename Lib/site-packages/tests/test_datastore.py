...
class TestDate(unittest.TestCase):
    ...
    def test_init(self):
        ...
        a_l = datastore.Date(int(self.ts_one))
...
class TestBytes(unittest.TestCase):
    ...
    def test_init(self):
        ...
        self.assertRaises(Exception, datastore.Bytes, 'hola')
    ...
    def test_comparisons(self):
        ...
        self.assertTrue(foo.__eq__('foo') is NotImplemented)
        self.assertTrue(foo.__ne__('foo') is NotImplemented)
        self.assertTrue(foo.__lt__('foo') is NotImplemented)
        self.assertTrue(foo.__le__('foo') is NotImplemented)
        self.assertTrue(foo.__gt__('foo') is NotImplemented)
        self.assertTrue(foo.__ge__('foo') is NotImplemented)
...
class TestSize(unittest.TestCase):
    ...
    def _assert_sizes(self, records, change_sizes):
        ...
        for (record, expected_value_size) in records:
            ...
            self.assertEqual(record.get_size(), expected_record_size,
                              'Size mismatch for %r with size %d != %d' % (record,
                                                                           record.get_size(),
                                                                           expected_record_size))
            total_size += expected_record_size

        self.assertEqual(self.datastore.get_record_count(), len(records))
        self.assertEqual(self.datastore.get_size(), total_size)
        if change_sizes:
            self.assertEqual(self.datastore.get_pending_changes_size(),
                              self.datastore.BASE_DELTA_SIZE +
                              sum((self.BASE_CHANGE_SIZE + change_size)
                                  for change_size in change_sizes))
        else:
            self.assertEqual(self.datastore.get_pending_changes_size(), 0)

    def test_compute_atom_size_edgecases(self):
        self.assertEqual(datastore._compute_atom_size(None), 0)
    ...
    def test_types(self):
        records = [(self.table.insert(ival=42), self.BASE_FIELD_SIZE),
                   (self.table.insert(lval=42), self.BASE_FIELD_SIZE),
                   ...
                   (self.table.insert(uval='\u1234'),
                    self.BASE_FIELD_SIZE + len('\u1234'.encode('utf-8'))),
    ...
    def test_record_ops(self):
        ...
        self.assertEqual(r1.get_size(), 0)
