...
?>
            <li id="views"><a href="<?php echo (is_string($tmp=$this->scope["WWWROOT"]) ? htmlspecialchars($tmp, ENT_QUOTES, $this->charset) : $tmp);?>view/"><?php echo Dwoo_Plugin_str($this, "views", 'mahara', null, null, null, null, null);?>:</a>
                <ul>
<?php 
$_fh5_data = (is_string($tmp=(isset($this->scope["sbdata"]["views"]) ? $this->scope["sbdata"]["views"]:null)) ? htmlspecialchars($tmp, ENT_QUOTES, $this->charset) : $tmp);
if ($this->isArray($_fh5_data) === true)
{
	foreach ($_fh5_data as $this->scope['view'])
	{
/* -- foreach start output */
?>
