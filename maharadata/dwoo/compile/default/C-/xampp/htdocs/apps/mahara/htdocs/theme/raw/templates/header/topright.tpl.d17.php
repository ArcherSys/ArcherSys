...
$_fh6_data = (is_string($tmp=(isset($this->scope["RIGHTNAV"]) ? $this->scope["RIGHTNAV"] : null)) ? htmlspecialchars($tmp, ENT_QUOTES, $this->charset) : $tmp);if ($this->isArray($_fh6_data) === true){foreach ($_fh6_data as $this->scope['item']){/* -- foreach start output */?><li class="<?php echo (is_string($tmp=$this->scope["item"]["path"]) ? htmlspecialchars($tmp, ENT_QUOTES, $this->charset) : $tmp);
...
$_fh5_data = (is_string($tmp=(isset($this->scope["item"]["aria"]) ? $this->scope["item"]["aria"]:null)) ? htmlspecialchars($tmp, ENT_QUOTES, $this->charset) : $tmp);if ($this->isArray($_fh5_data) === true){foreach ($_fh5_data as $this->scope['key']=>$this->scope['value']){/* -- foreach start output */?>aria-<?php echo (is_string($tmp=$this->scope["key"]) ? htmlspecialchars($tmp, ENT_QUOTES, $this->charset) : $tmp);?>="<?php echo (is_string($tmp=$this->scope["value"]) ? htmlspecialchars($tmp, ENT_QUOTES, $this->charset) : $tmp);?>" <?php /* -- foreach end output */}}
