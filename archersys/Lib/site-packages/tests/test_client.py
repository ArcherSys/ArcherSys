...
class BaseClientTests(object):
    ...
    def test_create_folder(self):
        ...
        path = posixpath.join(self.test_dir, "new_fold\xe9r")
    ...
    def test_create_folder_dupe(self):
        ...
        path = posixpath.join(self.test_dir,  "new_fold\xe9r_dupe")
        metadata = self.client.file_create_folder(path)
        self.assertRaises(
            ErrorResponse,
    ...
    def test_delete(self):
        ...
        path = posixpath.join(self.test_dir, "d\xe9lfoo.txt")
        self.upload_file(self.foo, path)
        metadata = self.client.metadata(path)
        self.assert_file(metadata, self.foo, path = path)
    ...
    def test_search(self):
        ...
        self.upload_file(self.foo, j(path, "t\xe9xt.txt"))
        ...
        all_tex_files = {j(path, n) for n in ["text.txt", "t\xe9xt.txt", "subFolder/text.txt"]}

        results = self.client.search(path, "tex")
        self.assertEqual({r["path"] for r in results}, all_tex_files)

        results = self.client.search(path, "t\xe9x")
        self.assertEqual({r["path"] for r in results}, all_tex_files)
